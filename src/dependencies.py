...
def get_city_repo(session: Session = Depends(get_db_session),  mapper = Depends(get_city_mapper)) -> CityRepository:
    return CityRepository(session, mapper)

def get_weather_repo(session: Session = Depends(get_db_session), mapper = Depends(get_weather_mapper)) -> WeatherRepository:
